...
• Production valuation via EZAUTO datahouse — seller data in, canonical spec + MYR valuation out, user updated in sell wizard.
...
This is the ongoing model once live: on a miss, we pull from our sources, normalise, merge, and store — the same storage rules the rollout applies in §3.

### 2.1 Source-agnostic ingestion

On every production valuate, CarInventory sends seller data to EZAUTO; EZAUTO returns canonical spec + MYR valuation (from its store, or upstream on miss). CarInventory persists to `vehicles`, `valuations`, and encrypted `source_records`, then updates the sell wizard. Prototype (`demo/`) uses `lib/valuation.ts` instead; production falls back to it only if EZAUTO is unreachable.
...
sequenceDiagram
    ...
    CarInventoryAPI->>CarInventoryDB: Insert vehicle (draft)
    CarInventoryAPI->>EZAUTO: Lookup
    alt Cache hit
        EZAUTO-->>CarInventoryAPI: Spec + valuation (MYR)
    else Cache miss
        EZAUTO->>Upstream: Fetch
        Upstream-->>EZAUTO: Spec, valuation
        EZAUTO-->>CarInventoryAPI: Spec + valuation (MYR)
    end
    CarInventoryAPI->>CarInventoryDB: source_record + valuation + spec update
...
| ------------------ | ----------------------------------------------------- | --------------------------------------- |
| Seller             | Plate, spec, condition, photos, mileage, asking price | Always                                  |
| EZAUTO             | Canonical spec, valuation (MYR)                       | Every production valuate                |
...
When two sources give different values for the same field, we need one rule that decides the winner — and it must be trivial to extend as new sources come on board. CarInventory defers upstream conflict resolution to EZAUTO. EZAUTO's trust table governs what arrives in the API response; CarInventory applies a second, simpler rule for seller-entered fields vs EZAUTO:


| Data type                                                  | Trusted source (default)                                                                      |
| ---------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Valuation (market / retail / trade price)                  | EZAUTO datahouse (upstream: Autograb per EZAUTO trust table)                                  |
| Spec (make, model, year, variant, transmission, fuel_type) | EZAUTO datahouse on valuate; seller intake pre-fills wizard only                              |
| Plate → vehicle identity                                   | Seller-entered plate triggers EZAUTO lookup; EZAUTO canonical spec wins                       |
| Condition grade                                            | Seller intake only — EZAUTO does not override condition in v1                                 |
| Mileage (km)                                               | Seller intake — applied as adjustment input; EZAUTO valuation may factor mileage via upstream |
